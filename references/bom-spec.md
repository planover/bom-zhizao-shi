...
| 字段 | 类型 | 约束 |
|------|------|------|
| product_name | string | 可选，默认空字符串 `""`；非空时在 Excel 中插入"产品名称"行 |
...
| materials[].erp_code | string | 可选，默认空字符串 `""`；物料在 ERP 系统中的编码 |
...
├───────────────┴───────────────┴──────────┴─────────────┤
│ 产品名称：芒果果味糖浆（A3:E3 合并，仅当 product_name 非空）│
├──────────────────────────────────────────────────────────┤
│ 一、物料信息                                              │
├──────────┬───────┬───────┬──────────┬──────────────────┤
│ 物料名称  │ 单位  │ 用量  │ 出品率(%) │ ERP物料代码       │
├──────────┼───────┼───────┼──────────┼──────────────────┤
│ ...物料行  │       │       │          │ RM-001            │
├──────────┴───────┴───────┴──────────┴──────────────────┤
│ 二、工艺工序                                            │
...
> **注意**：当 `product_name` 为空时不输出"产品名称"行，物料区/工序区起始行上移一行，布局与旧版完全一致（向后兼容）。
...
5. **产品名称**：扫描包含 `产品名称` 的单元格（即 `A3:E3` 合并区左上角 `A3`），提取冒号后内容；找不到则默认空字符串 `""`（旧版 Excel 无此行，向后兼容）。
6. **物料区**：定位含 `一、物料信息` 的行，其下一行为表头（忽略），再往下的数据行直到遇到含 `二、工艺工序` 的行或空行（物料名称列空即止）；每行取 col1=name、col2=unit、col3=usage(float)、col4=yield_rate(float)、col5=erp_code(str，空则 `""`）；**完全空行跳过**。
7. **工序区**：定位含 `二、工艺工序` 的行，其下一行为表头（忽略），再往下的数据行直到空行（工序编号为空即止）；每行取 col1=step_no、col2=name、col3=desc、col4=work_hours(float 或保留原值/空)、col5=note；**完全空行跳过**。
8. **数值转换**：`usage` / `yield_rate` / `work_hours` 尽量转 float（Excel 中出品率数字格式 `0.0"%"` 实际存储为原始数值，解析后即得到 float，如 `98.0`）；为空则保留空字符串 `""`。
