...
2. **阶段一 · 物料采集**：逐条录入物料，每条 7 个基础字段 + 按行业动态追加专属字段（电子：位号/封装/型号/RoHS；化工：CAS号/含量(%)/GHS标识；食品：过敏原）。
...
> 逆向导入是「重新编辑已有 BOM」的入口：导出的 JSON 可直接重新喂给正向流程。**注意**：「三、配料表/元件清单/配方表」为派生区块，逆向不重建实体；仅按物料名回收专属字段（过敏原/电子4字段/化工3字段）回写到物料对象，重新生成时按 `industry` 重新派生。V4 逆向输出 JSON 含 `industry` 字段（从区块标记或 category 推断）+ 7 个专属字段（默认空串补全）。
...
| 执行标准 | 否 | 默认 `""`；行业建议：食品→GB 7718-2025，电子→GB/T 39560，化工→GB/T 16483-2008；非空才在表头行 5 显示 |
...
- **电子（industry=="电子"）**：「三、元件清单」（8 列 A–H），排除"其他"类物料，按 (物料类型, 位号) 升序排序；表头 `序号/位号(Designator)/型号(Part#)/封装(Footprint)/物料名称/数量/物料类型/RoHS`；RoHS 着色：否→红字 FF0000，未知/空→黄字 BF8F00，是→默认。
- **化工（industry=="化工"）**：「三、配方表」（8 列 A–H），排除"包材"类物料，按含量(%) 降序排序；表头 `序号/物料名称/CAS号/含量(%)/GHS标识/物料类型/计量单位/用量`；含量(%) 数字格式 `0.0"%"`，空值留空。
- **其他行业**：不生成「三、」专属视图。

出品率/全产品出品率/用量占比%/含量(%) 单元格数字格式 `0.0"%"`。

> 说明：「序号」「用量占比%」「合计用量」均为派生展示，**不写入 JSON**；逆向导入时这些列被忽略。V4 逆向从「三、元件清单」按物料名回收 designator/footprint/part_number/rohs，从「三、配方表」回收 cas_number/concentration/ghs_hazard，从「三、配料表」回收过敏原；物料对象补全 7 个专属字段默认空串。
...
├── CHANGELOG.md              # 版本变更记录
├── scripts/
│   ├── bom_constants.py      # V4 共享常量模块（INDUSTRIES/CATEGORY_TO_INDUSTRY/过滤集/EDIBLE/ALLERGEN_*）
│   ├── generate_bom.py       # 正向：JSON -> BOM 表 Excel（V4，8 列 + 配料表/元件清单/配方表）
│   └── import_bom.py         # 逆向：BOM 表 Excel -> JSON（V4，按列头文本定位 + 专属字段回收）
├── references/
│   ├── bom-spec.md           # BOM 表 Excel 结构与输入 JSON Schema 规范（V4）
│   └── bom-demo.svg          # BOM 表 Excel 视觉演示图（含电子/化工专属区块）
└── examples/
