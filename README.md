...
1. **阶段零 · 产品信息**：采集产品名称（可选，可留空）、版本号（默认 V1.0）、生成日期（默认当天）。
   - 产品名称留空时 Excel 不输出"产品名称"行，布局与旧版一致。
2. **阶段一 · 物料采集**：逐条录入物料，每条 5 个字段（物料名称、计量单位、用量、出品率(%)、ERP物料代码（可选））。
   - 支持逐字段回复，也支持按格式整段回复；支持多条批量录入。
   - 每条校验通过后才存入临时列表；可随时「继续添加 / 结束」。
3. **阶段二 · 工序采集**：逐条录入工序，每条 5 个字段（工序编号、工序名称、工序说明、工时、备注）。
   - 工序编号可自动建议（上一条 +1）；允许 0 道工序（纯物料 BOM）。
4. **数据校验**：用量必须为正数、出品率须为 0–100 的正数、工时数值须 ≥0、工序编号不可重复等，逐项即时校验。
5. **汇总确认**：输出结构化汇总，确认后生成。
   ```
   ===== BOM 信息汇总（请确认）=====
   产品名称：芒果果味糖浆
   版本号：V1.0   生成日期：2026-07-07
   【物料】（共 N 条）
    1. 名称 / 单位 / 用量 / 出品率 / ERP代码
   【工序】（共 M 条）
    1. 编号 / 名称 / 说明 / 工时 / 备注
   =================================
...
```
用户：帮我生成一份 BOM 表
BOM智造师：请输入产品名称（如：芒果果味糖浆），可直接回复跳过：
用户：芒果果味糖浆
...
生成的 BOM 表为单工作表（标题 `BOM表`，5 列 A–E），含：合并标题行、版本号 / 生成日期行、**产品名称行（仅当 product_name 非空时出现）**、「一、物料信息」物料区（表头含 ERP物料代码列）、「二、工艺工序」工序区、相应表头与数据行；出品率单元格数字格式 `0.0"%"`。
...
`bom.json` 示例（含 product_name 与 erp_code）：
```json
{
  "product_name": "芒果果味糖浆",
  "version": "V1.0",
  "date": "2026-07-07",
  "materials": [
    {"name": "芒果", "unit": "kg", "usage": 46.3, "yield_rate": 55, "erp_code": "RM-001"}
  ],
  "processes": [
    {"step_no": "S01", "name": "调配", "desc": "混合搅拌", "work_hours": 30, "note": "常温"}
  ]
}
```
