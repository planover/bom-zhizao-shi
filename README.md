...
- **V5 向后兼容**：旧 JSON/Excel 缺 V5 专属字段（电子 10/化工 8/纺织 5/家具 4/成本 2）→ 默认空串，行为零变化；旧 8 列电子/化工 Excel 逆向仍可正确解析（按列头文本定位，max_col 向后兼容）；V5 不新增任何阻断/软校验，回归风险最低。机械/包装行业本期维持通用兜底（仅评估草案，不预置专属视图）。
...
├── CHANGELOG.md              # 版本变更记录
├── scripts/
│   ├── bom_constants.py      # V4/V5 共享常量模块（INDUSTRIES/CATEGORY_TO_INDUSTRY/过滤集/EDIBLE/ALLERGEN_*/INDUSTRY_TEMPLATES）
│   ├── generate_bom.py       # 正向：JSON -> BOM 表 Excel（V5，8 列 + 配料表/元件清单(14)/配方表(13)/面料辅料清单/家具物料清单/成本明细）
│   └── import_bom.py         # 逆向：BOM 表 Excel -> JSON（V5，按列头文本定位 + 全部专属字段回收）
├── references/
│   ├── bom-spec.md           # BOM 表 Excel 结构与输入 JSON Schema 规范（V5）
│   ├── bom-demo.svg          # BOM 表 Excel 视觉演示图（含电子/化工/纺织/家具/成本专属区块）
│   └── mechanical-packaging-draft-v5.md  # P2 机械/包装评估草案（本期不实现，仅评估）
└── examples/
